      ...
      query =
        ...
        when "PostgreSQL", "PostGIS"
          ...
          when :week # start on Sunday, not PostgreSQL default Monday
            ["(DATE_TRUNC('#{period}', (#{column}::timestamp - INTERVAL '#{week_start} day' - INTERVAL '#{day_start} second') AT TIME ZONE ?) + INTERVAL '#{week_start} day' + INTERVAL '#{day_start} second')", time_zone]
          else
            ["(DATE_TRUNC('#{period}', (#{column}::timestamp - INTERVAL '#{day_start} second') AT TIME ZONE ?) + INTERVAL '#{day_start} second')", time_zone]
          end
        when "SQLite"
